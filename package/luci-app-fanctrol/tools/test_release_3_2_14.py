...
def main() -> None:
    ...
    assert "var tempSourceKeys = ['cpu', 'wifi', 'modem', 'nvme'];" in view
    ...
    assert source_options == [
        ...
        ("wifi", "<%:Fan Source Wireless Network%>"),
        ("modem", "<%:Fan Source 5G Module%>"),
        ("nvme", "<%:Fan Source Solid State Drive%>"),
    ]
    assert 'aria-label="<%:Choose Temperature Sources%>" disabled' in view
    assert '<option value="cpu" disabled>' in source_selects[0]
    assert '<option value="wifi" hidden disabled>' in source_selects[0]
    assert '<option value="modem" hidden disabled>' in source_selects[0]
    assert '<option value="nvme" hidden disabled>' in source_selects[0]
    assert not re.search(r'<option\s+value="phy"', source_selects[0])
