...
modem_network_task()
{
    ...
    while true; do
        # USB modems can re-enumerate after a radio reset. Refresh the dynamic
        # paths instead of retaining stale ports or interface names.
        at_port=$(uci -q get modem.modem${modem_no}.at_port)
        manufacturer=$(uci -q get modem.modem${modem_no}.manufacturer | tr 'A-Z' 'a-z')
        ...
        # A single public resolver may rate-limit ICMP or be unreachable while
        # the modem data session is healthy. Require three failed rounds across
        # two independent targets before checking the modem connection state.
	if network_probe_ok "${interface_network}"; then
        ...
        # FM350 RNDIS can lose its IPv6 route after a long-running session.
        # Refresh router/neighbor discovery periodically for that exact modem;
        # other modem types must be left to their native netifd protocol.
